...
function ihacklog_get_latest_updates($num=5,$interval = 604800 ) 
{
	GLOBAL $wpdb;
	$num = (int) $num;
...
function ihackog_display_latest_updates($num=5,$interval_days = 7,$displayDate=1,$timeformat = "jS F'Y")
{ 
	$interval = 3600 * 24 * $interval_days ;
	$list =	ihacklog_get_latest_updates($num,$interval );
	if (!empty($list)) {
