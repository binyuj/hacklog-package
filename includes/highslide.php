...
add_filter('the_content', 'hlHighSlide_replace', 1);
add_filter('comment_text', 'hlHighSlide_replace', 1);
//add js to header
function hlHighSlide_add_js()
{
...
//按需加载JS，只能在hlHighSlide_add_js函数里面判断，在这里判断不行
add_action('wp_footer', 'hlHighSlide_add_js');



function highslide_stylesheets()
{
		wp_enqueue_style('highslide', plugin_dir_url(HACKLOG_PACKAGE_LOADER ) . 'js/highslide/highslide.min.css.php', array() , '4.1.12');

}
add_action('wp_print_styles','highslide_stylesheets');
